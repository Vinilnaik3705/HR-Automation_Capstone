...
class FeedbackAgent:
    ...
    def get_scheduled_interviews(self):
        """Get upcoming and recent interviews - FIXED VERSION"""
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT 
                        ...
                        -- Use interview_schedules candidate_name as primary, fallback to resume_data
    ...
    def save_feedback(self, interview_id: int, feedback_data: Dict, interviewer_id: int):
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    WHERE LOWER(candidate_email) = (
                        SELECT LOWER(candidate_email) 
                        FROM interview_schedules 
                        WHERE id = %s
                    )
                """, (recommendation, interview_id))
                
                conn.commit()
                
                self.send_candidate_update_email(interview_id, recommendation, feedback_data)
    ...
    def send_candidate_update_email(self, interview_id: int, recommendation: str, feedback_data: Dict):
        """Send email to candidate based on interview outcome"""
        ...
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    SELECT iss.candidate_name, iss.candidate_email, i.name as interviewer_name
                    FROM interview_schedules iss
                    JOIN interviewers i ON iss.interviewer_id = i.id
                    WHERE iss.id = %s
                """, (interview_id,))
    ...
    def _send_email(self, to_email: str, subject: str, body: str) -> bool:
        ...
        try:
            ...
            if not all([smtp_server, sender_email, sender_password]):
                st.info("📧 Email not configured - would send:")
...
def show_feedback_section():
    """Main feedback collection interface"""
    ...
    with tab4:
        show_analytics_tab(feedback_agent)

def show_submit_feedback_tab(feedback_agent: FeedbackAgent):
    """Tab for submitting interview feedback"""
    st.subheader("Submit Interview Feedback")
    ...
    with st.form("feedback_form"):
        ...
        if submitted:
            ...
            success = feedback_agent.save_feedback(
                selected_feedback['id'], feedback_data, interviewer_id
            )
            
            if success:
                st.success("✅ Feedback submitted successfully!")
                st.success("📧 Candidate notification email sent!")
            else:
                st.error("Failed to submit feedback")

def show_view_interviews_tab(feedback_agent: FeedbackAgent):
    """Tab for viewing all interviews"""
    ...
    filtered_df = df[df['Status'].str.lower().isin([s.lower() for s in status_filter])]
    st.dataframe(filtered_df, use_container_width=True)

def show_candidate_status_tab(feedback_agent: FeedbackAgent):
    """Tab for viewing candidate status"""
...
def show_analytics_tab(feedback_agent: FeedbackAgent):
    """Tab for feedback analytics"""
    ...
    with col1:
        st.metric("Total Interviews", stats['overall']['total_interviews'])
    with col2:
        st.metric("Completed", stats['overall']['completed'])
    with col3:
        st.metric("Scheduled", stats['overall']['scheduled'])
    with col4:
        st.metric("Feedback Submitted", stats['overall']['feedback_submitted'])
    
    st.write("### Recommendation Breakdown")
    if stats['recommendations']:
        rec_df = pd.DataFrame(stats['recommendations'])
        st.bar_chart(rec_df.set_index('interview_status')['count'])
    
    st.write("### Average Ratings")
    if stats['ratings']:
        ratings = stats['ratings']
        rating_data = {
            'Category': ['Technical', 'Communication', 'Problem Solving', 'Cultural Fit', 'Overall'],
            'Score': [
                ratings['avg_technical'] or 0,
                ratings['avg_communication'] or 0,
                ratings['avg_problem_solving'] or 0,
                ratings['avg_cultural_fit'] or 0,
                ratings['avg_overall'] or 0
            ]
        }
        rating_df = pd.DataFrame(rating_data)
        st.bar_chart(rating_df.set_index('Category')['Score'])

def create_feedback_tables():
    """Create feedback-related database tables"""
    conn = None
    try:
