...
class InterviewScheduler:
        ...
        try:
            if os.path.exists("secrets.toml"):
                ...
                if not google_config:
                    #st.info("🔒 Google Calendar not configured - using database scheduling")
                    return
    ...
    def get_db_connection(self):
        """Reuse database connection from main app"""
        try:
            if os.path.exists("secrets.toml"):
                secrets = toml.load("secrets.toml")
    ...
    def get_available_interviewers(self):
        """Get list of available interviewers from database"""
    ...
    def get_interviewer_availability(self, interviewer_id: int, date: datetime):
        ...
        try:
            ...
            start_time = datetime.combine(date, datetime.strptime(interviewer.get('working_hours_start', '09:00'), '%H:%M').time())
            end_time = datetime.combine(date, datetime.strptime(interviewer.get('working_hours_end', '17:00'), '%H:%M').time())
    ...
    def get_default_availability(self, date: datetime):
        """Generate default availability when calendar integration is not available"""
        slots = []
        ...
        while current_time < end_time:
            ...
            current_time += timedelta(minutes=45)  # 30 min slot + 15 min buffer
    ...
    def schedule_interview(self, candidate_email: str, candidate_name: str, 
        ...
        """Schedule an interview and send calendar invite"""
        try:
            interviewer = self.get_interviewer_by_id(interviewer_id)
            if not interviewer:
            ...
            if self.calendar_service:
                event_id = self.create_calendar_event(
                    candidate_name, candidate_email, interviewer, slot, interview_duration
                )
            ...
            if conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        RETURNING id
                    """, (candidate_name, candidate_email, interviewer_id, 
                          slot['start'], interview_duration, event_id))
                    
                    schedule_id = cur.fetchone()[0]
                    conn.commit()
            
            if self.email_config:
                self.send_interview_invitation(
                    candidate_name, candidate_email, interviewer, slot, interview_duration
                )
            
            self.log_scheduling_action(
                f"Scheduled interview for {candidate_name} with {interviewer['name']}"
            )
    ...
    def create_calendar_event(self, candidate_name: str, candidate_email: str,
        ...
        try:
            event = {
                ...
                'end': {
                    'dateTime': (slot['start'] + timedelta(minutes=duration)).isoformat(),
                    'timeZone': interviewer.get('timezone', 'UTC'),
                },
                'attendees': [
    ...
    def send_interview_invitation(self, candidate_name: str, candidate_email: str,
        ...
        try:
            ...
            Interview Details:
            - Date: {slot['start'].strftime('%B %d, %Y')}
    ...
    def send_rescheduling_notification(self, candidate_name: str, candidate_email: str,
        ...
        try:
            ...
            - Date: {new_slot['start'].strftime('%B %d, %Y')}
            - Time: {new_slot['start'].strftime('%I:%M %p')}
    ...
    def log_scheduling_action(self, action: str):
        """Log scheduling actions to database"""
        conn = self.get_db_connection()
...
def show_interview_scheduling_section():
    """Main interview scheduling interface"""
...
def show_schedule_interview_tab(scheduler: InterviewScheduler):
    """Tab for scheduling new interviews"""
...
def show_scheduled_interviews_tab(scheduler: InterviewScheduler):
    """Tab for viewing scheduled interviews"""
...
def show_reschedule_cancel_tab(scheduler: InterviewScheduler):
    """Tab for rescheduling or cancelling interviews"""
...
def show_interviewer_setup_tab(scheduler: InterviewScheduler):
    """Tab for setting up interviewers"""
    ...
    with col1:
        ...
        with st.form("add_interviewer_form"):
            ...
            timezone = st.selectbox("Timezone", ["UTC", "US/Eastern", "US/Central", "US/Pacific", "Europe/London", "Asia/Kolkata"])
            ...
            if st.form_submit_button("Add Interviewer", use_container_width=True):
                if name and email:
                    success = save_interviewer_to_db(
                        name, email, calendar_id, timezone, 
        ...
        if interviewers:
            for interviewer in interviewers:
                with st.expander(f"{interviewer['name']} ({interviewer['email']})"):
                    ...
                    st.write(f"Working Hours: {interviewer.get('working_hours_start', '09:00')} - {interviewer.get('working_hours_end', '17:00')}")
                    st.write(f"Status: {'Active' if interviewer.get('is_active', True) else 'Inactive'}")
        else:
            st.info("No interviewers configured")

def save_interviewer_to_db(name: str, email: str, calendar_id: str, timezone: str,
                          working_start: str, working_end: str, buffer_minutes: int, is_active: bool):
    """Save interviewer to database"""
    conn = None
    try:
...
def create_scheduling_tables():
    """Create necessary database tables for scheduling"""
    conn = None
    try:
        conn = scheduler.get_db_connection()
        if conn:
...
def main_with_scheduling():
    """Modified main function to include scheduling"""

    st.sidebar.title("Navigation")
    app_mode = st.sidebar.selectbox(
        "Choose a module",
        ["Upload Resumes", "Parse & View", "Job Match", "AI Chat", "Interview Scheduling", "History & Analytics"]
    )
